...
\midrule
Пересечение & $100\,000 \times 100$ & Sequential & 318000 нс & 447 КБ & 1.00 (Эталон) \\
Пересечение & $100\,000 \times 100$ & Traversal & 83000 нс & 86 КБ & $\approx 3.8\times$ (Ускорение) \\
\midrule
Разность & $100 \times 100\,000$ & Sequential & 243000 нс & 321 КБ & 1.00 (Эталон) \\
Разность & $100 \times 100\,000$ & Traversal & 75387000 нс & 88 МБ & $\approx 309\times$ (Замедление) \\
\midrule
Пересечение & $10\,000 \times 100\,000$ & Sequential & 15.69 мс & 17.9 МБ & 1.00 (Эталон) \\
Пересечение & $10\,000 \times 100\,000$ & Parallel (4 threads) & 7.80 мс & 18.0 МБ & $\approx 2.0\times$ (Ускорение) \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Одиночные операции}
Анализ результатов замеров показывает, что характер роста времени выполнения базовых операций очень похож на логарифмический. Сложность операций вставки и поиска в сбалансированном АВЛ-дереве составляет $O(\log N)$. То есть при увеличении размера множества с $N_1 = 100$ до $N_2 = 100\,000$ время выполнения теоретически должно вырасти в $2,5$ раза. На практике время выполнения операции добавления выросло с 582~нс до 1376~нс (коэффициент роста $\approx 2,36$), что даже чуть ниже ожидаемого роста. Этот результат вполне согласуется с поведением деревьев и их высот. Объем выделяемой памяти изменяется от 840 до 2080 байт на операцию.
...
\caption{Зависимость времени выполнения от выбранного подхода. Ось ординат имеет логарифмический масштаб.}
\label{fig:traversal}
\end{figure}

Метод \verb|Traversal| демонстрирует более высокую производительность при $|A| \gg |B|$. В сценарии пересечения множеств размеров $100\,000$ и $100$ время выполнения составило $83900$~нс против $318200$~нс для \verb|Sequential|. При $|A| \ll |B|$ использование обхода приводит к снижению производительности (например, $75,38$~мс против $0,24$~мс для операции разности).
...
\caption{Время выполнения пересечения (A=10k, B=100k) в зависимости от потоков. Пунктирной линией отмечено время последовательного выполнения.}
\label{fig:parallel}
\end{figure}
...
Как видно из результатов, при пересечении множеств $10\,000 \times 100\,000$ базовый последовательный алгоритм выполняется за $15,69$~мс. Использование двух потоков снижает время до $9,95$~мс, а четырех потоков~--- до $7,80$~мс, обеспечивая двукратное ускорение. Благодаря порогу отсечения удалось полностью нивелировать накладные расходы планировщика ОС и устранить избыточную нагрузку на сборщик мусора: объем выделяемой памяти в параллельном варианте стабилизировался на уровне $18$~МБ, что практически идентично последовательной реализации.
